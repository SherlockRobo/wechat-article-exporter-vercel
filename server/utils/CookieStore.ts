...
import { openMpSession } from '~/server/utils/MpSessionToken';
...
export async function getCookieFromStore(event: H3Event): Promise<string | null> {
  const sealedSession = getRequestHeader(event, 'X-Mp-Session');
  const session = openMpSession(sealedSession);
  if (session) {
    return AccountCookie.create(session.token, session.cookies).toString();
  }
