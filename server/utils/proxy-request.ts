...
import { sealMpSession } from '~/server/utils/MpSessionToken';
...
export async function proxyMpRequest(options: RequestOptions) {
  ...
  const extraResponseHeaders = new Headers();
  ...
  // 返回给客户端的一个 auth-key 的 cookie
    ...
    try {
      ...
      extraResponseHeaders.set('X-Mp-Session', sealMpSession(cookieStore.store.get(authKey)!.toJSON()));
  ...
  extraResponseHeaders.forEach((value, key) => {
